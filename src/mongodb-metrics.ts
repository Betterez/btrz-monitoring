...
const poolSize = new metrics.Gauge({
...
const checkedOutConnections = new metrics.Gauge({
...
const maxPoolSize = new metrics.Gauge({
...
const waitQueueSize = new metrics.Gauge({
...
const checkOutFailuresTotal = new metrics.Counter({
...
**/
function normalizeCheckOutFailureReason(reason: string | Error | undefined): string {
 ...
 */
function readConfiguredMaxPoolSize(db: MongoDbClient): number {
  try {
