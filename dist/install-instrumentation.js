...
function initializeMonitoring(options) {
    ...
    const { enabled = true, samplePercentage = DEFAULT_SAMPLE_PERCENTAGE, serviceName, metricsPort, ignoreStaticAssetDir = [], ignoredHttpMethods = [], ignoredRoutes = [], ignoredAwsSqsEvents = [], enableFilesystemTracing = false, } = tracingOptions;
...
function shutdownMonitoring(sdk) {
    return async () => {
        try {
